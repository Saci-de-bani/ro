...
#
# NodeConn: an object which manages p2p connectivity to a ripoffcoin node
# NodeConnCB: a base class that describes the interface for receiving
#             callbacks with network messages from a NodeConn
# CBlock, CTransaction, CBlockHeader, CTxIn, CTxOut, etc....:
#     data structures that should map to corresponding structures in
#     ripoffcoin/primitives
...
# Objects that map to ripoffcoind objects, which can be serialized/deserialized
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(ripoffcoinhash(r))
            self.hash = encode(ripoffcoinhash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in ripoffcoind indicates these should be deserialized as blocks
